...
export default function TravelerRegister2() {
  ...
  return (
    ...
    <div className="max-w-xl mx-auto px-6 py-12 pt-12 space-y-10 text-center text-sm">
     ...
     <button onClick={() => alert("회원가입이 완료되었습니다.")} 
     className="w-20 bg-gray-300 px-6 py-2 rounded-full hover:bg-gray-400 font-medium">확인</button>
    </div>
    </div>
  );
}
      ...
      {options.map(opt => (
        <button
          ...
          className={`px-4 py-1 rounded-full border transition ${
            selected.includes(opt) ? 'bg-green-400 text-white border-green-400' : 'bg-gray-100 text-gray-700 hover:bg-green-100'
          }`}
        >
          {opt}
        </button>
